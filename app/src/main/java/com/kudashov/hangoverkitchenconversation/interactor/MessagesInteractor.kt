...
class MessagesInteractor(
    ...
    fun getMessages(token: String, roomId: String): Single<List<Message>> {
        return repository.getMessages(token, roomId)
    }
    ...
    fun sendMessages(
        ...
        return repository.sendMessages(token, roomId, text, isAnonymous, photos)
    }

    fun observeMessages(token: String, roomId: String): Observable<Message> {
        return repository.observeMessages(token, roomId)
    }
